<?php
    ...
    require_once(__DIR__.'/getFriendIds.php');
    require_once(__DIR__.'/getConnection.php');
    require_once(__DIR__.'/getUsersInfo.php');

    //自分のidからフォローしている人のidを検索
    if(isset($_GET['next_cursor'])){
        $next_cursor = $_GET['next_cursor'];
        $friends = getFriendIds($_SESSION['user_id'],200,$next_cursor);
    }else{
        $friends = getFriendIds($_SESSION['user_id'],200);
    }
    ...
    if($friends['error']['flag'] == true){
        ...
        $friends = getConnection($friends['ids']);
        if($friends['error']['flag'] == true){
            $friend_datas['error']['flag'] = true;
            $friend_datas['error']['message'] = $friends['error']['message'];
        }else{
